...
static bool inClassCMode = false;
static MOTE_MODE_T moteState = MOTE_STARTUP;

typedef enum {runningEnter, runningSensor, runningSensorMenu, runningDataRateDisplay, runningDataRateSelect, runningSensorDisplay, runningUplink, runningUplinkSelect, runningUplinkTransmit, runningPortNumber, runningDownlink, runningDownlinkDisplay, runningMenuAux, runningClassCReception, runningMenuTimeout, runningPeriodicWakeUpLength, /* runningPeriodic, */runningPeriodicDisplay, runningSleep} MOTE_RUNNING_T;
...
static uint8_t SleepTimeOut = 10;        // In Secs; This is for Menu Timout
...
static MOTE_RUNNING_T moteRunningProcess(bool changeStates, bool selectButton, bool messageReady, bool sleepEvent)
{
    ...
    if ((sleepEvent) && (periodicState))
    {
        ...
        {   
            ...
            //moteApp_add16bToDataBuffer(RH/10, 4);
            moteApp_add8bToDataBuffer(temperature, 9);
            //moteApp_add8bToDataBuffer(Temp/10, 9);
            NOP();
    ...
    switch(runningState)
    {
        ...
        case runningMenuTimeout:
            ...
            {
                oled_clear();
                oled_putString(" Allow Class C  ",0,0);
                oled_putString("(NAV)    (SEL) ",0,3);
                //oled_putString("       ",0,1);
                //oled_putString("Every:",0,2);
                //oled_putString("Mins",12,2);
                // Toggle Display
                runningState = runningMenuAux;
            }
            else if (selectButton)
            {
                if (menuTimeout)
        ...
        case runningMenuAux: //Class C
            ...
            else if (selectButton)
            {
                    ...
                    __delay_ms(16);
                    __delay_ms(16);
                    sendCommand("radio rx 9800");
                    moteApp_delayms(500);
                    //menuTimeout = false;
                    runningState = runningClassCReception;
            }
        break;

//lg
        case runningClassCReception:
            if ( messageReady )
            {
                //menuTimeout=false;
                SleepTimeOut = 30;
                ...
                {
                    oled_putString("Reception timeout",0,3);
                    LED_GREEN_LAT = LED_OFF;
                    //menuTimeout=true;
                    runningState = runningUplink;
                    moteApp_delayms(4000);
        ...
        case runningPeriodicWakeUpLength:
            if (changeStates)
            {
                ...
                switch (dataRateValue)
                {
                    ...
                    break;
                    case 6:
                        oled_putString(" 6 ",10,2);
                    break;
                    case 7:
                        oled_putString(" 7 ",10,2);
                    break;
                    case 8:
...
void StartSignal(void)
{
 ...
 TRISBbits.TRISB7=0;  //Configure RB0 as output
 PORTBbits.RB7=0;     //RB0 sends 0 to the sensor
 __delay_ms(9);
 __delay_ms(9);
 PORTBbits.RB7= 1;    //RB0 sends 1 to the sensor
 __delay_us(30);
 TRISBbits.TRISB7= 1;    //Configure RB0 as input
  }
...
char ReadData(void)
{
 ...
 for(j = 0; j < 8; j++)
 {
    while(!PORTBbits.RB7); //Wait until PORTB.RB0 goes HIGH
    __delay_us(30);
    if (PORTBbits.RB7 == 0)
        {i=i & (~(1<<(7 - j)));  //Clear bit (7-b)
        }
    else 
        {
        i= i| (1 << (7 - j));  //Set bit (7-b)
        while(PORTBbits.RB7);
        }  //Wait until PORTB.RB0 goes LOW
 }
 return i;
 }

void ReadAndProcessDHT22(void) 
{
 WPUBbits.WPUB7=1;   
  ...
  if(Sum == ((RH_byte1+RH_byte2+T_byte1+T_byte2) & 0XFF))
    ...
    oled_putUint16(Temp / 10,10,2);
    
    oled_putUint16(RH/10,10,3);
    /*
    //LCD_Chr(1, 12, 48 + ((Temp / 100) % 10));
    oled_putUint8((Temp / 100) % 10,12,2);
    //LCD_Chr(1, 13, 48 + ((Temp / 10) % 10));
    oled_putUint8((Temp / 10) % 10,13,2);
    //LCD_Chr(1, 15, 48 + (Temp % 10));
    oled_putUint8((Temp % 10),15,2);

    //LCD_Chr(2, 12, 48 + ((RH / 100) % 10));
    oled_putUint8(((RH / 100) % 10),12,3);
    //LCD_Chr(2, 13, 48 + ((RH / 10) % 10));
    oled_putUint8(((RH / 10) % 10),13,3);
    //LCD_Chr(2, 15, 48 + (RH % 10));
    oled_putUint8((RH % 10),15,3);
    */
    }
  else
    {
    oled_putString("Check sum error ",0,3);
    }
  }
  else 
  {
  oled_putString("  No response   ",0,2);
  oled_putString(" from the sensor",0,3);
  }
